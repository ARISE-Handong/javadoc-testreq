...
public class TransformerUtils {
	 ...
	 * The transformer at array location 0 is called if the predicate at array location 0 returned true.
	 * Each predicate is evaluated until one returns true.
	 * If no predicates evaluate to true, null is returned.
     *
     * @param <I>  the input type
     * @param <O>  the output type
	 ...
	 * The transformer at array location 0 is called if the predicate at array location 0 returned true. 
	 * Each predicate is evaluated until one returns true. 
	 * If no predicates evaluate to true, the default transformer is called. 
     ...
     * Create a new Transformer that calls one of the transformers depending on the predicates.
     * <p>
     * The Map consists of Predicate keys and Transformer values. 
	 * A transformer is called if its matching predicate returns true.
	 * Each predicate is evaluated until one returns true.
	 * If no predicates evaluate to true, the default transformer is called.
	 * The default transformer is set in the map with a null key.
	 * If no default transformer is set, null will be returned in a default case.
	 * The ordering is that of the iterator() method on the entryset collection of the map.
     *
     * @param <I>  the input type
     * @param <O>  the output type
     ...
     * <p>
     * The Map consists of object keys and Transformer values.
	 * A transformer is called if the input object equals the key.
	 * If there is no match, the default transformer is called.
	 * The default transformer is set in the map using a null key.
	 * If no default is set, null will be returned in a default case.
     *
     * @param <I>  the input type
     * @param <O>  the output type
	 ...
	 * The constructor used is determined by the arguments specified to this method.
     *
     * @param <T>  the output type
     ...
     * @return the transformer, or {@link ConstantTransformer#nullTransformer()} if the {@code map} is {@code null}
     * @see MapTransformer
     */
	 ...
	 * If the input object is {@code null}, {@code null} is returned.
     *
     * <p>
     * For example, <code>TransformerUtils.invokerTransformer("getName");</code> will call the <code>getName</code> method on the input object to determine the transformer result.
     * </p>
     *
     * @param <I>  the input type
     * @param <O>  the output type
    ...
    /**
     * Gets a transformer that returns a <code>java.lang.String</code> representation of the input object.
	 * This is achieved via the <code>toString</code> method, <code>null</code> returns 'null'.
     *
     * @param <T>  the input type
     * @return the transformer
