...
public class CollectionUtils {
     ...
     * </p>
     * <p>
     * <b>Note:</b> from version 4.1 onwards this method requires the input collections and equator to be of compatible type (using bounded wildcards).
     * Providing incompatible arguments (e.g. by casting to their rawtypes) will result in a {@code ClassCastException} thrown at runtime.
     * </p>
     *
     ...
     * </p>
     * <ul>
     * <li> Map -- the value returned is the <code>Map.Entry</code> in position <code>index</code> in the map's <code>entrySet</code> iterator, if there is such an entry.</li>
     * <li> List -- this method is equivalent to the list's get method.</li>
     * <li> Array -- the <code>index</code>-th array entry is returned, if there is such an entry; otherwise an <code>IndexOutOfBoundsException</code> is thrown.</li>
     * <li> Collection -- the value returned is the <code>index</code>-th object returned by the collection's default iterator, if there is such an element.</li>
     * <li> Iterator or Enumeration -- the value returned is the <code>index</code>-th object in the Iterator/Enumeration, if there is such an element.
	 *		The Iterator/Enumeration is advanced to <code>index</code> (or to the end, if <code>index</code> exceeds the number of entries) as a side effect of this method.</li>
     * </ul>
     *
     ...
     * <p>
     * This method uses the {@link BoundedCollection} interface to determine the full status. 
	 * If the collection does not implement this interface then false is returned.
     * </p>
     * <p>
     * The collection does not have to implement this interface directly.
     * If the collection has been decorated using the decorators subpackage then these will be removed to access the BoundedCollection.
     * </p>
     *
     ...
     * <p>
     * This method uses the {@link BoundedCollection} interface to determine the maximum size.
	 * If the collection does not implement this interface then -1 is returned.
     * </p>
     * <p>
     * The collection does not have to implement this interface directly.
     * If the collection has been decorated using the decorators subpackage then these will be removed to access the BoundedCollection.
     ...
     * @param a  the first collection, must not be null
     * @param b  the second collection, must not be null
     * @param includeDuplicates  if {@code true} duplicate elements will be retained, otherwise they will be removed in the output collection
     * @return a new sorted List, containing the elements of Collection a and b
     * @throws NullPointerException if either collection is null
     * @since 4.0
     */
     ...
     * <p>
     * NOTE: the number of permutations of a given collection is equal to n!, where n is the size of the collection.
	 * Thus, the resulting collection will become <b>very</b> large for collections &gt; 10 (e.g. 10! = 3628800, 15! = 1307674368000).
     * </p>
     * <p>
    ...
    //-----------------------------------------------------------------------
    /**
     * Returns a collection containing all the elements in <code>collection</code> that are also in <code>retain</code>.
	 *  The cardinality of an element <code>e</code> in the returned collection is the same as the cardinality of <code>e</code> in <code>collection</code> unless <code>retain</code> does not contain <code>e</code>, in which case the cardinality is zero.
	 * This method is useful if you do not wish to modify the collection <code>c</code> and thus cannot call <code>c.retainAll(retain);</code>.
     * <p>
	 ...
	 * If it's contained, it's added to the returned list. As a consequence, it is advised to use a collection type for <code>retain</code> that provides a fast (e.g. O(1)) implementation of {@link Collection#contains(Object)}.
     * </p>
     *
     * @param <C>  the type of object the {@link Collection} contains
	 ...
	 * This method is useful if you do not wish to modify the collection <code>c</code> and thus cannot call <code>c.retainAll(retain);</code>.
     * <p>
     * Moreover this method uses an {@link Equator} instead of {@link Object#equals(Object)} to determine the equality of the elements in <code>collection</code> and <code>retain</code>.
	 * Hence this method is useful in cases where the equals behavior of an object needs to be modified without changing the object itself.
     * </p>
     *
	 ...
	 * That is, this method returns a collection containing all the elements in <code>c</code> that are not in <code>remove</code>.
	 ...
	 * This method is useful if you do not wish to modify the collection <code>c</code> and thus cannot call <code>collection.removeAll(remove);</code>.
     * <p>
     ...
     * @return a <code>Collection</code> containing all the elements of <code>collection</code> except any elements that also occur in <code>remove</code>.
     * @throws NullPointerException if either parameter is null
     ...
     * It is important not to use the original collection after invoking this method, as it is a backdoor for adding invalid objects.
     * </p>
     *
     ...
     * <p>
     * Each object is passed through the transformer as it is added to the Collection.
	 * It is important not to use the original collection after invoking this method, as it is a backdoor for adding untransformed objects.
     * </p>
     * <p>
