...
public class IndexSearcher {
   ...
   * User code and query implementations should respect {@link IndexSearcher#getSimilarity()}.
   * @lucene.internal
   */
   ...
   *  NOTE: if you are using {@link NIOFSDirectory}, do not use the shutdownNow method of ExecutorService as this uses Thread.interrupt under-the-hood which can silently close file descriptors (see <a href="https://issues.apache.org/jira/browse/LUCENE-2239">LUCENE-2239</a>).
   * 
   * @lucene.experimental */
   ...
   * <p>
   * Given a non-<code>null</code> {@link Executor} this method runs searches for each segment separately, using the provided Executor.
   * NOTE: if you are using {@link NIOFSDirectory}, do not use the shutdownNow method of ExecutorService as this uses Thread.interrupt under-the-hood which can silently close file descriptors (see <a href="https://issues.apache.org/jira/browse/LUCENE-2239">LUCENE-2239</a>).
   * 
   ...
   * A value of {@code null} indicates that query matches should never be cached.
   * This method should be called <b>before</b> starting using this {@link IndexSearcher}.
   * <p>NOTE: When using a query cache, queries should not be modified after they have been passed to IndexSearcher.
   * @see QueryCache
   * @lucene.experimental
   */
  ...
  /**
   * Return the query cache of this {@link IndexSearcher}.
   * This will be either the {@link #getDefaultQueryCache() default query cache} or the query cache that was last set through {@link #setQueryCache(QueryCache)}.
   * A return value of {@code null} indicates that caching is disabled.
   * @lucene.experimental
   */
  ...
  /**
   * Return the query cache of this {@link IndexSearcher}. 
   * This will be either the {@link #getDefaultQueryCachingPolicy() default policy} or the policy that was last set through {@link #setQueryCachingPolicy(QueryCachingPolicy)}.
   * @lucene.experimental
   */
   ...
   * Each {@link LeafSlice} is executed in a single thread. 
   * By default there will be one {@link LeafSlice} per leaf ({@link org.apache.lucene.index.LeafReaderContext}).
   */
  ...
  /** Expert: Get the {@link Similarity} to use to compute scores. 
   *  This returns the {@link Similarity} that has been set through {@link #setSimilarity(Similarity)} or the default {@link Similarity} if none has been set explicitly. */
  ...
  /** Returns the leaf slices used for concurrent searching, or null if no {@code Executor} was passed to the constructor.
   *
   * @lucene.experimental */
  ...
  /** Finds the top <code>n</code> hits for <code>query</code> where all results are after a previous result (<code>after</code>).
   * <p>
   * By passing the bottom result from a previous page as <code>after</code>, this method can be used for efficient 'deep-paging' across potentially large result sets.
   *
   ...
   * Finds the top <code>n</code> hits for <code>query</code>, and sorting the hits by the criteria in <code>sort</code>.
   * If <code>doDocScores</code> is <code>true</code> then the score of each hit will be computed and returned.
   * If <code>doMaxScore</code> is <code>true</code> then the maximum score over all collected hits will be computed.
   * 
   * @throws BooleanQuery.TooManyClauses If a query would exceed {@link BooleanQuery#getMaxClauseCount()} clauses.
   */
  ...
  /** Finds the top <code>n</code> hits for <code>query</code> where all results are after a previous result (<code>after</code>), allowing control over whether hit scores and max score should be computed.
   * <p>
   * By passing the bottom result from a previous page as <code>after</code>, this method can be used for efficient 'deep-paging' across potentially large result sets.
   * If <code>doDocScores</code> is <code>true</code> then the score of each hit will be computed and returned.
   * If <code>doMaxScore</code> is <code>true</code> then the maximum score over all collected hits will be computed.
 ...
 /**
  * Lower-level search API.
  * Search all leaves using the given {@link CollectorManager}.
  * In contrast to {@link #search(Query, Collector)}, this method will use the searcher's {@link Executor} in order to parallelize execution of the collection on the configured {@link #leafSlices}.
  ...
  */
   ...
   *
   * <p>This is intended to be used in developing Similarity implementations, and, for good performance, should not be displayed with every hit.
   * Computing an explanation is as expensive as executing the query over the entire index.
   */
   ...
   * Returns an Explanation that describes how <code>doc</code> scored against <code>weight</code>.
   *
   * <p>This is intended to be used in developing Similarity implementations, and, for good performance, should not be displayed with every hit.
   * Computing an explanation is as expensive as executing the query over the entire index.
  ...
  /**
   * Creates a {@link Weight} for the given query, potentially adding caching if possible and configured.
   * @lucene.experimental
   */
  ...
  /**
   * A class holding a subset of the {@link IndexSearcher}s leaf contexts to be executed within a single thread.
   * 
   * @lucene.experimental
   */
  ...
  /**
   * Returns {@link TermStatistics} for a term, or {@code null} if the term does not exist.
   ...
   *
   * @param docFreq     The document frequency of the term. 
  ...
  /**
   * Returns {@link CollectionStatistics} for a field, or {@code null} if the field does not exist (has no indexed terms)
   * 
