...
public final class CodecUtil {
   ...
   * This header can be parsed and validated with  {@link #checkHeader(DataInput, String, int, int) checkHeader()}.
   * <p>
   * CodecHeader --&gt; Magic,CodecName,Version
   * <ul>
   *    <li>Magic --&gt; {@link DataOutput#writeInt Uint32}. 
   *        This identifies the start of the header. It is always {@value #CODEC_MAGIC}.
   *    <li>CodecName --&gt; {@link DataOutput#writeString String}. 
   *        This is a string to identify this file.
   *    <li>Version --&gt; {@link DataOutput#writeInt Uint32}. 
   *        Records the version of the file.
   * </ul>
   * <p>
   * Note that the length of a codec header depends only upon the name of the codec, so this length can be computed at any time with {@link #headerLength(String)}.
   * 
   * @param out Output stream
   * @param codec String to identify this file. 
   *		It should be simple ASCII, less than 128 characters in length.
   ...
   * </ul>
   * <p>
   * Note that the length of an index header depends only upon the name of the codec and suffix, so this length can be computed at any time with {@link #indexHeaderLength(String,String)}.
   * 
   * @param out Output stream
   * @param codec String to identify the format of this file.
   *		It should be simple ASCII,  less than 128 characters in length.
   * @param id Unique identifier for this particular file instance.
   * @param suffix auxiliary suffix information for the file.
   *		It should be simple ASCII, less than 256 characters in length.
   * @param version Version number
   * @throws IOException If there is an I/O error writing to the underlying medium.
   * @throws IllegalArgumentException If the codec name is not simple ASCII, or is more than 127 characters in length, or if id is invalid, or if the suffix is not simple ASCII, or more than 255 characters in length.
   */
  ...
  /**
   * Reads and validates a header previously written with  {@link #writeHeader(DataOutput, String, int)}.
   * <p>
   * When reading a file, supply the expected <code>codec</code> and an expected version range (<code>minVersion to maxVersion</code>).
   * 
   ...
   * @return The actual version found, when a valid header is found that matches <code>codec</code>, with an actual version where {@code minVersion <= actual <= maxVersion}.
   *         Otherwise an exception is thrown.
   * @throws CorruptIndexException If the first four bytes are not {@link #CODEC_MAGIC}, or if the actual codec found is not <code>codec</code>.
   * @throws IndexFormatTooOldException If the actual version is less than <code>minVersion</code>.
   * @throws IndexFormatTooNewException If the actual version is greater than <code>maxVersion</code>.
  ...
  /**
   * Reads and validates a header previously written with  {@link #writeIndexHeader(DataOutput, String, int, byte[], String)}.
   * <p>
   * When reading a file, supply the expected <code>codec</code>, expected version range (<code>minVersion to maxVersion</code>), and object ID and suffix.
   * 
   * @param in Input stream, positioned at the point where the header was previously written.
   *		Typically this is located at the beginning of the file.
   * @param codec The expected codec name.
   ...
   * @return The actual version found, when a valid header is found  that matches <code>codec</code>, with an actual version where {@code minVersion <= actual <= maxVersion}, and matching <code>expectedID</code> and <code>expectedSuffix</code> Otherwise an exception is thrown.
   * @throws CorruptIndexException If the first four bytes are not {@link #CODEC_MAGIC}, or if the actual codec found is not <code>codec</code>, or if the <code>expectedID</code> or <code>expectedSuffix</code> do not match.
   * @throws IndexFormatTooOldException If the actual version is less  than <code>minVersion</code>.
   * @throws IndexFormatTooNewException If the actual version is greater  than <code>maxVersion</code>.
   * @throws IOException If there is an I/O error reading from the underlying medium.
   * @see #writeIndexHeader(DataOutput, String, int, byte[],String)
  ...
  /**
   * Expert: verifies the incoming {@link IndexInput} has an index header and that its segment ID matches the expected one, and then copies that index header into the provided {@link DataOutput}.  
   ...
   * @throws CorruptIndexException If the first four bytes are not {@link #CODEC_MAGIC}, or if the <code>expectedID</code> does not match.
   * @throws IOException If there is an I/O error reading from the underlying medium.
   *
   * @lucene.internal 
   */
   ...
   *  This throws {@link CorruptIndexException} if this file does not appear to be an index file. */
  ...
  /** Retrieves the full footer from the provided {@link IndexInput}.
   *  This throws {@link CorruptIndexException} if this file does not have a valid footer. */
   ...
   * This footer can be parsed and validated with  {@link #checkFooter(ChecksumIndexInput) checkFooter()}.
   * <p>
   * CodecFooter --&gt; Magic,AlgorithmID,Checksum
   * <ul>
   *    <li>Magic --&gt; {@link DataOutput#writeInt Uint32}. 
   *        This identifies the start of the footer. It is always {@value #FOOTER_MAGIC}.
   *    <li>AlgorithmID --&gt; {@link DataOutput#writeInt Uint32}. 
   *        This indicates the checksum algorithm used. Currently this is always 0, for zlib-crc32.
   *    <li>Checksum --&gt; {@link DataOutput#writeLong Uint64}. 
   *        The actual checksum value for all previous bytes in the stream, including the bytes from Magic and AlgorithmID.
   * </ul>
   * 
   * @param out Output stream
   * @throws IOException If there is an I/O error writing to the underlying medium.
   ...
   * @throws IOException if the footer is invalid, if the checksum does not match, or if {@code in} is not properly positioned before the footer at the end of the stream.
   */
  ...
  /** 
   * Validates the codec footer previously written by {@link #writeFooter}, optionally passing an unexpected exception that has already occurred.
   * <p>
   * When a {@code priorException} is provided, this method will add a suppressed exception indicating whether the checksum for the stream passes, fails, or cannot be computed, and rethrow it.
   * Otherwise it behaves the same as {@link #checkFooter(ChecksumIndexInput)}.
   * <p>
   * Example usage:
