...
public class PiePlot extends Plot implements Cloneable, Serializable {
	 ...
	 * If this is <code>null</code>, the background is not filled.
     */
    ...
    /**
     * This debug flag controls whether or not an outline is drawn showing the interior of the plot region.
	 *  This is drawn as a lightGray rectangle showing the padding provided by the 'interiorGap' setting.
     */
    static final boolean DEBUG_DRAW_INTERIOR = false;

    /**
     * This debug flag controls whether or not an outline is drawn showing the link area (in blue) and link ellipse (in yellow). 
	 * This controls where the label links have 'elbow' points.
     */
    ...
    /**
     * Creates a new plot. 
	 * The dataset is initially set to <code>null</code>.
     */
    public PiePlot() {
        this(null);
    }
    ...
    /**
     * Sets the interior gap and sends a {@link PlotChangeEvent} to all registered listeners. 
	 * This controls the space between the edges of the pie plot and the plot area itself (the region where the section labels appear).
     *
    ...
    /**
     * Sets a flag that controls whether <code>null</code> values are ignored, and sends a {@link PlotChangeEvent} to all registered listeners. 
	 * At present, this only affects whether or not the key is presented in the legend.
     *
     * @param flag  the flag.
     *
    ...
    /**
     * Sets a flag that controls whether zero values are ignored, and sends a {@link PlotChangeEvent} to all registered listeners. 
	 * This only affects whether or not a label appears for the non-visible pie section.
     *
     * @param flag  the flag.
     *
	 ...
	 * This is equivalent to <code>lookupSectionPaint(section, getAutoPopulateSectionPaint())</code>.
     *
    ...
    /**
     * Returns the paint for the specified section. 
	 * The lookup involves these steps:
     * <ul>
     ...
     * <li>if {@link #getSectionPaint(int)} is <code>null</code> but <code>autoPopulate</code> is <code>true</code>, attempt to fetch a new paint from the drawing supplier ({@link #getDrawingSupplier()});
     * <li>if all else fails, return {@link #getBaseSectionPaint()}.
     * </ul>
     *
     * @param key  the section key.
     * @param autoPopulate  a flag that controls whether the drawing supplier is used to auto-populate the section paint settings.
     *
     * @return The paint.
     *
     * @since 1.0.3
     ...
     *
     * @deprecated Use {@link #getSectionPaint(Comparable)} and {@link #getBaseSectionPaint()}. 
	 ...
	 * If this is set to {@code null}, then a list of paints is used instead (to allow different colors to be used for each section).
     *
     * @param paint  the paint (<code>null</code> permitted).
     *
     * @see #getSectionPaint()
     *
     * @deprecated Use {@link #setSectionPaint(Comparable, Paint)} and {@link #setBaseSectionPaint(Paint)}. 
	 * Deprecated as of version 1.0.6.
     */
	 ...
	 * If there is no such section in the dataset, we generate a key.
	 *  This is to provide some backward compatibility for the (now deprecated) methods that get/set attributes based on section indices. 
	 * The preferred way of doing this now is to link the attributes directly to the section key (there are new methods for this, starting from version 1.0.3).
     *
    ...
    /**
     * Clears the section paint settings for this plot and, if requested, sends a {@link PlotChangeEvent} to all registered listeners. 
	 * Be aware that if the <code>autoPopulateSectionPaint</code> flag is set, the section paints may be repopulated using the same colours as before.
     *
     * @param notify  notify listeners?
     *
     * @since 1.0.11
	 ...
	 *  This is used when no other paint is defined, which is rare.
    ...
    /**
     * Sets the base section paint and sends a {@link PlotChangeEvent} to all registered listeners.
     *
    ...
    /**
     * Sets the flag that controls whether or not the outline is drawn for each pie section, and sends a {@link PlotChangeEvent} to all registered listeners.
     *
	 ...
	 * This is equivalent to <code>lookupSectionPaint(section, getAutoPopulateSectionOutlinePaint())</code>.
     *
     * @param key  the section key.
     *
     * @return The paint for the specified section.
	 ...
	 * The lookup involves these steps:
     * <ul>
     * <li>if {@link #getSectionOutlinePaint()} is non-<code>null</code>, return it;</li>
     * <li>otherwise, if {@link #getSectionOutlinePaint(int)} is non-<code>null</code> return it;</li>
     * <li>if {@link #getSectionOutlinePaint(int)} is <code>null</code> but <code>autoPopulate</code> is <code>true</code>, attempt to fetch a new outline paint from the drawing supplier ({@link #getDrawingSupplier()});
     * <li>if all else fails, return {@link #getBaseSectionOutlinePaint()}.
     * </ul>
     *
     * @param key  the section key.
     * @param autoPopulate  a flag that controls whether the drawing supplier is used to auto-populate the section outline paint settings.
     *
     * @return The paint.
     *
     * @since 1.0.3
    ...
    /**
     * Clears the section outline paint settings for this plot and, if requested, sends a {@link PlotChangeEvent} to all registered listeners.
     * Be aware that if the <code>autoPopulateSectionPaint</code> flag is set, the section paints may be repopulated using the same colours as before.
     *
     * @param notify  notify listeners?
     *
    ...
    /**
     * Returns the base section paint.
	 * This is used when no other paint is available.
     *
     * @return The paint (never <code>null</code>).
     *
	 ...
	 * This is equivalent to <code>lookupSectionOutlineStroke(section, getAutoPopulateSectionOutlineStroke())</code>.
     *
     * @param key  the section key.
     *
	 ...
	 * The lookup involves these steps:
     * <ul>
     * <li>if {@link #getSectionOutlineStroke()} is non-<code>null</code>, return it;</li>
     * <li>otherwise, if {@link #getSectionOutlineStroke(int)} is non-<code>null</code> return it;</li>
     * <li>if {@link #getSectionOutlineStroke(int)} is <code>null</code> but <code>autoPopulate</code> is <code>true</code>, attempt to fetch a new outline stroke from the drawing supplier ({@link #getDrawingSupplier()});
     * <li>if all else fails, return {@link #getBaseSectionOutlineStroke()}.
     * </ul>
     *
     * @param key  the section key.
     * @param autoPopulate  a flag that controls whether the drawing supplier is used to auto-populate the section outline stroke settings.
     *
     * @return The stroke.
     *
     * @since 1.0.3
    ...
    /**
     * Clears the section outline stroke settings for this plot and, if requested, sends a {@link PlotChangeEvent} to all registered listeners.
     * Be aware that if the <code>autoPopulateSectionPaint</code> flag is set, the section paints may be repopulated using the same colours as before.
     *
     * @param notify  notify listeners?
     *
	 ...
	 * This is used when no other stroke is available.
     *
    ...
    /**
     * Sets the section label generator and sends a {@link PlotChangeEvent} to all registered listeners.
     *
    ...
    /**
     * Sets the gap between the edge of the pie and the labels (expressed as a percentage of the plot width) and sends a {@link PlotChangeEvent} to all registered listeners.
     *
    ...
    /**
     * Sets the flag that controls whether or not label linking lines are visible and sends a {@link PlotChangeEvent} to all registered listeners.
     * Please take care when hiding the linking lines - depending on the data values, the labels can be displayed some distance away from the corresponding pie section.
     *
     * @param visible  the flag.
     *
    ...
    /**
     * Sets the paint used for the lines that connect pie sections to their corresponding labels, and sends a {@link PlotChangeEvent} to all registered listeners.
     *
     * @param paint  the paint (<code>null</code> not permitted).
     *
    ...
    /**
     * Sets the flag that controls whether simple or extended labels are displayed on the plot, and sends a {@link PlotChangeEvent} to all registered listeners.
     *
	 ...
	 * If the generator is <code>null</code>, no tool tips will be created.
     *
     * @return The generator (possibly <code>null</code>).
     *
	 ...
	 * Pie sections for an angle smaller than this are not drawn, to avoid a JDK bug.
     *
	 ...
	 * Pie sections for an angle smaller than this are not drawn, to avoid a JDK bug.  See this link for details:
     * <br><br>
     * <a href="http://www.jfree.org/phpBB2/viewtopic.php?t=2707"> http://www.jfree.org/phpBB2/viewtopic.php?t=2707</a>
     * <br><br>
     * ...and this bug report in the Java Bug Parade:
     * <br><br>
    ...
    /**
     * Sets the legend label generator and sends a {@link PlotChangeEvent} to all registered listeners.
     *
	 ...
	 * Note that this is a bitmap drop-shadow generation facility and is separate from the vector based show option that is controlled via the {@link #setShadowPaint(java.awt.Paint)} method.
     *
     * @param generator  the generator (<code>null</code> permitted).
     *
     * @since 1.0.14
    ...
    /**
     * Handles a mouse wheel rotation (this method is intended for use by the {@code MouseWheelHandler} class).
     *
	 ...
	 * This method will be called before the first item is rendered, giving the plot an opportunity to initialise any state information it wants to maintain.
     *
     ...
     *
     * @param leftKeys  a collection of keys and angles (to the middle of the section, in degrees) for the sections on the left side of the plot.
     * @param g2  the graphics device.
     * @param plotArea  the plot area.
     * @param linkArea  the link area.
     ...
     * Checks to see if the section is exploded and recalculates the new center if so.
     *
    ...
    /**
     * Returns the paint for the specified section.
	 * This is equivalent to <code>lookupSectionPaint(section)</code>.
	 * Checks to see if the user set the Paint to be of type RadialGradientPaint If so it adjusts the center and radius to match the Pie
     *
     * @param key  the section key.
     * @param state  PiePlotState.
     *
	 ...
	 * Note that the plot's dataset is NOT included in the test for equality.
     *
     ...
     *
     * @deprecated This field is redundant, it is sufficient to use sectionPaintMap and baseSectionPaint.
	 *  Deprecated as of version 1.0.6.
     */
     ...
     *
     * @deprecated This field is redundant, it is sufficient to use sectionOutlinePaintMap and baseSectionOutlinePaint.
	 *  Deprecated as of version 1.0.6.
     */
     ...
     *
     * @deprecated This field is redundant, it is sufficient to use sectionOutlineStrokeMap and baseSectionOutlineStroke.
	 *  Deprecated as of version 1.0.6.
     */
    ...
    /**
     * Sets the paint used to fill a section of the pie and sends a {@link PlotChangeEvent} to all registered listeners.
     *
     * @param section  the section index (zero-based).
     * @param paint  the paint (<code>null</code> permitted).
     *
     ...
     *
     * @deprecated Use {@link #getSectionOutlinePaint(Comparable)} and {@link #getBaseSectionOutlinePaint()}. 
	 * Deprecated as of version 1.0.6.
     */
     ...
     *
     * @deprecated Use {@link #setSectionOutlinePaint(Comparable, Paint)} and {@link #setBaseSectionOutlinePaint(Paint)}.
	 *  Deprecated as of version 1.0.6.
     */
     ...
     * @param paint  the paint (<code>null</code> permitted).
     *
     * @deprecated Use {@link #setSectionOutlinePaint(Comparable, Paint)} instead.
     */
     ...
     *
     * @deprecated Use {@link #getSectionOutlineStroke(Comparable)} and {@link #getBaseSectionOutlineStroke()}.
	 *  Deprecated as of version 1.0.6.
     */
     ...
     *
     * @deprecated Use {@link #setSectionOutlineStroke(Comparable, Stroke)} and {@link #setBaseSectionOutlineStroke(Stroke)}.
	 *  Deprecated as of version 1.0.6.
     */
    ...
    /**
     * Sets the stroke used to fill a section of the pie and sends a {@link PlotChangeEvent} to all registered listeners.
     *
     * @param section  the section index (zero-based).
     * @param stroke  the stroke (<code>null</code> permitted).
     *
     * @deprecated Use {@link #setSectionOutlineStroke(Comparable, Stroke)} instead.
     */
