...
public class CollectionUtilsTest extends MockTestCase {
    ...
    public void forAllDoCollection() {
        ...
        col.add(collectionA);
        col.add(collectionC);
        Closure<Collection<Integer>> resultClosure = CollectionUtils.forAllDo(col, (Closure<Collection<Integer>>) null);
        assertNull(resultClosure);

        assertTrue(collectionA.isEmpty() && collectionC.isEmpty());

        final Closure<Collection<Integer>> testClosure = ClosureUtils.invokerClosure("clear");
        Closure<Collection<Integer>> resultClosure = CollectionUtils.forAllDo((Collection<Collection<Integer>>) null, testClosure);
        col.add(null);
        // null should be OK
    ...
    public void getFromMap() {
        ...
        map.put("zeroKey", "zero");
        map.put("oneKey", "one");
        Map.Entry<String, String> test = CollectionUtils.get(map, 0);
...
//Up to here
    ...
    public void collect() {
        ...
        final Transformer<Number, Long> transformer = TransformerUtils.constantTransformer(2L);
        Collection<Number> collection = CollectionUtils.<Integer, Number>collect(iterator, transformer);
        assertTrue(collection.size() == collectionA.size());
        ...
        Collection<Number> collection = CollectionUtils.collect((Iterator<Integer>) null, (Transformer<Integer, Number>) null);
        assertTrue(collection.size() == 0);

        final int size = collectionA.size();
        Collection<Number> collectionB = CollectionUtils.collect((Collection<Integer>) null, transformer, collectionB);
    ...
    public void testCollate() {
        ...
        List<Integer> result = CollectionUtils.collate(collectionA, emptyCollection);
        ...
        List<Integer> result = CollectionUtils.collate(emptyCollection, emptyCollection, reverseComparator);
        ...
        List<Integer> result1 = CollectionUtils.collate(collectionD, collectionE, reverseComparator);
        rList<Integer> result2 = CollectionUtils.collate(collectionE, collectionD, reverseComparator);
